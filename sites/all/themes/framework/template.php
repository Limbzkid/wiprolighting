...
function get_payback_dropdown_list() {
	...
	$query->leftJoin('field_data_field_base_name', 'p', 'p.entity_id = n.nid');
	$query->fields('n', array('nid', 'status', 'title'))
				->fields('p', array('entity_id', 'field_base_name_value'))
				->condition('n.type', array('payback_calculator_matrix'), 'IN')
				->condition('n.status', 1, '=');	
	$result = $query->execute();
	foreach($result as $row) {
		if(!in_array($row->field_base_name_value, $temp_arr) && $row->field_base_name_value != '') {
...
function get_sub_apps($term_id) {   // 
    ...
    $output = '<div class="formRight"><div class="form"><h2>Get a <span class="semiBold">Call Back</span></h2>';
		$output .= '<span class="thankYouMsg">Thank you for getting in touch with us. We will get back to you shortly.</span>';
		$output .= '<form type="POST" action="'.$action.'" autocomplete="off"><div class="formData">';
    $output .= '<div class="row"><div class="formIco"><span class="spireIcon nameIco"></span></div><div class="field">';
		$output .= '<input id="name" type="text" value="'.$name.'" placeholder="*Name" maxlength="50" minlength="2" autocomplete="false">';
		$output .= '<span class="errorVisible">Please enter valid name</span></div></div>';
		$output .= '<div class="row"><div class="formIco"><span class="spireIcon mailIco"></span></div><div class="field">';
		$output .= '<input id="email" type="text" value="'.$mail.'" placeholder="*E-mail" maxlength="50" minlength="6" autocomplete="false">';
		$output .= '<span class="errorVisible">Please enter valid email</span></div></div>';
		$output .= '<div class="row"><div class="formIco"><span class="spireIcon mobileIco"></span></div><div class="field">';
		$output .= '<input id="mobile" type="tel" value="'.$contact.'" placeholder="*Mobile Number" maxlength="10" minlength="10" autocomplete="false">';
		$output .= '<span class="errorVisible">Please enter valid mobile number</span></div></div>';
		$output .= '<div class="row"><div class="formIco"><span class="spireIcon cityIco"></span></div><div class="field">';
		$output .= '<input id="city" type="text" value="" placeholder="*City" maxlength="25" minlength="3" autocomplete="false">';
		$output .= '<span class="errorVisible">Please enter valid city name</span></div></div>';
		$output .= '<div class="row capchBox"><div class="clm"><div class="field">';
		$output .= '<div class="g-recaptcha" data-sitekey="6LfdChsTAAAAAMedO_GDChZ3BAGTFM7Cj6ar0ieY"></div>';
		$output .= '<span class="captchaErr dnone"></span> </div></div></div>';
		$output .= '<div class="row"> <a id="submitThis" class="arrowLink formBtn" href="javascript:;">Send <span class="arrowIco"></span></a><div class=" dnone submitLoader"><img src="'.$base_url.'/sites/all/themes/framework/images/submit_loader.gif" id="" class=""/></div></div></div>';
		$output .= '</form></div></div>';
    return $output;
  }
  
  function getLightingAppAreas($area) {
		$sql = "SELECT DISTINCT(ttd.name), CONVERT(SUBSTRING(ttd.name, LOCATE(' ', ttd.name) + 1), SIGNED INTEGER) FROM field_data_field_area fa 
						...
						ORDER BY CAST(ttd.name AS UNSIGNED) ASC";
		$result = db_query($sql);
		$options = '';
  ...
  function getLightingAppMountingHeight($arg) {
	...
	function get_LEDEDGE_popup_box() {
		$output = '<div class="lightbox lbLedEdge"><a class="closeBtn"></a><div class="lightboxContent"><h2>LED<span class="semiBold">EDGE</span></h2>';
		$output .= '<ul class="ledGroup">';
		$output .= '<li class="eco"><span class="firstLetter">E</span>co - sustainability</li>';
		$output .= '<li class="design"><span class="firstLetter">D</span>esign &amp; innovation</li>';
		$output .= '<li class="green"><span class="firstLetter">G</span>reen Technology</li>';
		$output .= '<li class="effi"><span class="firstLetter">E</span>fficient lighting solutions</li>';
		$output .= '</ul>';
		$output .= '<p>Wipro Lighting\'s LED Edge offers an opportunity to discover the benefits of good lighting with futuristic, functional and eco-sustainable lighting solutions. LED EDGE provides a healthy lighting environment to the people in every way.</p>';
		$output .= '<div class="ledEdgeAdvantages"> <span class="titleWrap"><h3>LEDEDGE <span class="semiBold">advantages</span></h3></div>';
		$output .= '<img class="imgCall" src="'.base_path().'sites/default/files/staticimgs/lededge-advantage-img.jpg" alt="" /></div></div>';
		return $output;
	}
